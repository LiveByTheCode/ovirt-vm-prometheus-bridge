...
RUN apk add --update curl && cd / && curl -L -O $SOURCE_URL && gunzip $NAME.tar.gz && tar xf $NAME.tar && mv $NAME/bin/ovirt-vm-prometheus-bridge / && rm -rf $NAME.tar.gz $NAME && apk del curl && rm -rf /var/cache/apk/*
